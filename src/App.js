...
function FriendsList({ friends, onSelection,selectedFriend }) {
  return <ul>
    {friends.map( friend=> <Friend selectedFriend={ selectedFriend} onSelection={onSelection} key={friend.id} friend={friend} />)}
  </ul>
}
...
function Friend({ friend, onSelection, selectedFriend }) {
  return <li className={selectedFriend?.id === friend?.id ? "selected" : ""}>
    ...
    { friend.balance > 0 && <p className="green">You owe {friend.name} owes you {Math.abs(friend.balance)}</p>}
...
function FormSplitBill({selectedFriend, onSplitBill}) {
  ...
  return <form className="form-split-bill" onSubmit={handleSubmit}>
    ...
    <input onChange={e=>setBill(Number(e.target.value))} type="text" />
